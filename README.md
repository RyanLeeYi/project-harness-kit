...
| # | 檢查 | 擋住什麼 |
|---|------|----------|
| 1 | **acceptance 凍結** — 已簽核的驗收標準被改動，且 commit 沒帶簽核 trailer | 把標準改鬆讓它過 |
...
| 5 | **schema 自檢** — 讀不到預期欄位就 **FAIL-CLOSED 並明講** | 防線無聲失效卻看起來很健康 |
...
12 個情境，每一種該擋的與不該擋的都跑一遍（含 schema 缺欄位、JSON 壞掉這種「防線自己失效」的情況）。改過 `check.ps1` 就跑一次。
